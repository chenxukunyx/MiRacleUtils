...
public class AppCharacterParser {
    ...
    public static AppCharacterParser getInstace() {
    ...
    //将汉子转为ascii码
    private int getChrASCII(String chr) {
        int src = 0;
        try {
            byte[] bytes = chr.getBytes("gb2312");
            if (bytes == null || bytes.length > 2 || bytes.length <= 0) {
                throw new RuntimeException("illeage resource string");
            }
            if (bytes.length == 1) {
                src = bytes[0];
            }
            if (bytes.length == 2) {
                int highByte = 256 + bytes[1];
                int lowByte = 256 + bytes[0];
                src = (256 * highByte + lowByte) - 256 * 256;
            }
